use std::{path::PathBuf, sync::mpsc::{self, Receiver, TryRecvError}};
use anyhow::{Context, Result};
use eframe::egui::{self, Color32, RichText, Vec2, Pos2, Rect, Sense, Stroke};
use vibeshop::{document::{self, Blend, Document, Editor, Layer}, gpu::Engine, image_io};
...
enum Tool { Hand, Move }
enum Action { Open(Option<PathBuf>, bool), Exit }
enum Job { Opened(Layer, bool), Exported(PathBuf), Cancelled }
...
impl Studio {
    pub fn new(cc: &eframe::CreationContext<'_>) -> Result<Self> {
        theme(&cc.egui_ctx);
        let state = cc.wgpu_render_state.as_ref().context("Vibeshop requires a WebGPU-compatible graphics device")?.clone();
        let adapter = state.adapter.get_info();
        Ok(Self { editor: Editor::new(document::demo_document()), gpu: Engine::new(state.device.clone(), state.queue.clone()),
            render_state: state, texture: None, rendered_revision: 0, tool: Tool::Hand, zoom: 1.0, pan: Vec2::ZERO, fit: true,
            move_start: None, job: None, pending: None, allow_close: false,
            status: "Generated demo · open a photo to make it yours".into(), error: None,
            adapter: format!("{} · {:?}", adapter.name, adapter.backend) })
    }
    fn render(&mut self) {
        if self.rendered_revision == self.editor.revision { return; }
        match self.gpu.render(&self.editor.document) {
            ...
            Err(error) => { self.error = Some(format!("{error:#}")); self.rendered_revision = self.editor.revision; }
        }
    }
    fn request(&mut self, action: Action, ctx: &egui::Context) {
        if self.job.is_some() { self.status = "Finish the current file operation first".into(); return; }
        if self.editor.dirty && !matches!(action, Action::Open(_, true)) { self.pending = Some(action); }
        else { self.execute(action, ctx); }
    }
    fn execute(&mut self, action: Action, ctx: &egui::Context) {
        match action {
            Action::Exit => { self.allow_close = true; ctx.send_viewport_cmd(egui::ViewportCommand::Close); }
            Action::Open(path, add) => {
                let (tx, rx) = mpsc::sync_channel(1); self.job = Some(rx); self.status = "Opening image…".into();
                let ctx = ctx.clone();
                std::thread::spawn(move || {
                    let result = (|| {
                        let path = path.or_else(|| pollster::block_on(rfd::AsyncFileDialog::new().add_filter("Images", &["png", "jpg", "jpeg"]).pick_file()).map(|f| f.path().to_path_buf()));
                        match path { Some(path) => Ok(Job::Opened(image_io::open(&path)?, add)), None => Ok(Job::Cancelled) }
                    })();
    ...
    fn export(&mut self, ctx: &egui::Context) {
        ...
        self.render();
        let snapshot = match self.gpu.readback() { Ok(s) => s, Err(e) => { self.error = Some(e.to_string()); return; } };
        let (tx, rx) = mpsc::sync_channel(1); self.job = Some(rx); self.status = "Exporting PNG…".into();
        let ctx = ctx.clone();
        std::thread::spawn(move || {
            let result = (|| {
                let Some(file) = pollster::block_on(rfd::AsyncFileDialog::new().add_filter("PNG", &["png"]).set_file_name("vibeshop.png").save_file()) else { return Ok(Job::Cancelled); };
    ...
    fn poll_job(&mut self) {
        let result = match self.job.as_ref().map(|r| r.try_recv()) {
            ...
            Some(Err(TryRecvError::Disconnected)) => Err(anyhow::anyhow!("File worker stopped unexpectedly")),
            _ => return,
        };
        self.job = None;
        match result {
            Ok(Job::Opened(layer, add)) => {
                ...
                if layer.source.width > limit || layer.source.height > limit { self.error = Some(format!("Image exceeds this GPU's {limit}px texture limit")); return; }
                if add { if let Err(e) = self.editor.add_layer(layer) { self.error = Some(e.to_string()); return; } }
                else { self.editor.replace(Document::new(layer)); self.fit = true; }
                self.status = "Image opened locally · source pixels are preserved".into();
            }
            Ok(Job::Exported(path)) => self.status = format!("Exported {} · editable layers remain in memory", path.file_name().unwrap_or_default().to_string_lossy()),
            Ok(Job::Cancelled) => self.status = "File operation cancelled".into(),
            Err(error) => { self.status = "File operation failed".into(); self.error = Some(format!("{error:#}")); }
        }
    }
    fn shortcuts(&mut self, ctx: &egui::Context) {
        if ctx.wants_keyboard_input() || self.pending.is_some() || self.error.is_some() { return; }
        let command = ctx.input(|i| i.modifiers.command);
        let shift = ctx.input(|i| i.modifiers.shift);
        if command {
            if ctx.input(|i| i.key_pressed(egui::Key::O)) { self.request(Action::Open(None, shift), ctx); }
            if ctx.input(|i| i.key_pressed(egui::Key::S)) { self.export(ctx); }
            if ctx.input(|i| i.key_pressed(egui::Key::Z)) { if shift { self.editor.redo(); } else { self.editor.undo(); } }
        } else {
            if ctx.input(|i| i.key_pressed(egui::Key::H)) { self.tool = Tool::Hand; }
            if ctx.input(|i| i.key_pressed(egui::Key::V)) { self.tool = Tool::Move; }
            if ctx.input(|i| i.key_pressed(egui::Key::F)) { self.fit = true; }
        }
    }
    fn top_bar(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::top("toolbar").exact_height(66.0).frame(egui::Frame::new().fill(PANEL).inner_margin(egui::Margin::symmetric(20, 12))).show(ctx, |ui| {
            ui.horizontal_centered(|ui| {
                ui.label(RichText::new("vibe").size(25.0).strong().color(ACCENT));
                ui.label(RichText::new("shop").size(25.0).strong());
                ui.add_space(24.0);
                if ui.add_enabled(self.job.is_none(), egui::Button::new("Open image")).on_hover_text("Open PNG or JPEG · Ctrl/Cmd+O").clicked() { self.request(Action::Open(None, false), ctx); }
                if ui.add_enabled(self.job.is_none(), egui::Button::new("+ Add layer")).on_hover_text("Ctrl/Cmd+Shift+O").clicked() { self.request(Action::Open(None, true), ctx); }
                ui.separator();
                if ui.add_enabled(self.editor.can_undo(), egui::Button::new("Undo")).on_hover_text("Ctrl/Cmd+Z").clicked() { self.editor.undo(); }
                if ui.add_enabled(self.editor.can_redo(), egui::Button::new("Redo")).on_hover_text("Ctrl/Cmd+Shift+Z").clicked() { self.editor.redo(); }
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.add_enabled(self.job.is_none(), egui::Button::new(RichText::new("Export PNG  ↗").color(Color32::from_rgb(24, 31, 19)).strong()).fill(ACCENT).min_size(egui::vec2(140.0, 34.0))).on_hover_text("Export current pixels · Ctrl/Cmd+S").clicked() { self.export(ctx); }
                    ui.add_space(12.0); ui.label(RichText::new("LOCAL FIRST").size(10.0).color(MUTED));
                });
            });
        });
        egui::TopBottomPanel::top("document").exact_height(38.0).frame(egui::Frame::new().fill(Color32::from_rgb(23, 25, 29)).inner_margin(egui::Margin::symmetric(20, 8))).show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.label(RichText::new("CANVAS").size(10.0).color(ACCENT)); ui.add_space(12.0);
                let d = &self.editor.document;
                ui.label(format!("{} × {} px", d.width, d.height));
                ui.label(RichText::new("sRGB input · linear 16F composition").size(11.0).color(MUTED));
                if self.editor.dirty { ui.label(RichText::new("●  Unsaved layers").color(ACCENT).size(11.0)); }
            });
        });
    }
    ...
    fn canvas(&mut self, ctx: &egui::Context) {
        egui::SidePanel::left("tools").exact_width(68.0).resizable(false).frame(egui::Frame::new().fill(PANEL).inner_margin(10)).show(ctx, |ui| {
            ui.add_space(15.0);
            for (tool, key, label) in [(Tool::Hand, "H", "Pan"), (Tool::Move, "V", "Move")] {
                if ui.add_sized([46.0, 42.0], egui::Button::new(RichText::new(key).strong().size(18.0)).selected(self.tool == tool)).on_hover_text(format!("{label} · {key}")).clicked() { self.tool = tool; }
                ui.vertical_centered(|ui| { ui.label(RichText::new(label).size(10.0).color(MUTED)); }); ui.add_space(18.0);
            }
        });
        egui::CentralPanel::default().frame(egui::Frame::new().fill(Color32::from_rgb(16, 18, 22)).inner_margin(0)).show(ctx, |ui| {
            ...
            if self.fit { self.zoom = ((rect.width() - 100.0) * pixels_per_point / dimensions.x).min((rect.height() - 110.0) * pixels_per_point / dimensions.y).clamp(0.02, 16.0); self.pan = Vec2::ZERO; }
            if response.hovered() {
            ...
            let panning = self.tool == Tool::Hand || ctx.input(|i| i.key_down(egui::Key::Space));
            if response.dragged() && panning { self.pan += response.drag_delta(); self.fit = false; }
            if response.drag_started() && !panning {
                if let (Some(pointer), Some(layer)) = (ctx.input(|i| i.pointer.press_origin()), self.editor.document.layers.get(self.editor.selected)) {
                    self.move_start = Some((pointer, layer.offset)); self.editor.begin_edit();
                }
            }
            if response.dragged() && !panning {
                if let (Some((start, offset)), Some(pointer)) = (self.move_start, response.interact_pointer_pos()) {
                    ...
                    if let Some(layer) = self.editor.document.layers.get_mut(self.editor.selected) { if layer.offset != next { layer.offset = next; self.editor.changed(); } }
                }
            }
            if response.drag_stopped() { self.move_start = None; self.editor.finish_edit(); }
            if response.double_clicked() { self.fit = true; }
            ...
            painter.rect_stroke(image, 0.0, Stroke::new(1.0, Color32::from_gray(76)), egui::StrokeKind::Outside);
            painter.text(rect.left_top() + egui::vec2(24.0, 22.0), egui::Align2::LEFT_TOP, "YOUR NEXT GOOD IDEA STARTS HERE", egui::FontId::proportional(10.0), MUTED);
            painter.text(rect.center_bottom() - egui::vec2(0.0, 24.0), egui::Align2::CENTER_BOTTOM, "Scroll to zoom   ·   Space + drag to pan   ·   F to fit", egui::FontId::proportional(11.0), MUTED);
        });
    }
    fn dialogs(&mut self, ctx: &egui::Context) {
        if self.pending.is_some() {
            let mut proceed = false; let mut cancel = false;
            ...
            if proceed { if let Some(action) = self.pending.take() { self.execute(action, ctx); } }
        }
        if let Some(error) = self.error.clone() {
            let mut dismiss = false;
            egui::Window::new("Could not complete that").collapsible(false).resizable(false).anchor(egui::Align2::CENTER_CENTER, Vec2::ZERO).show(ctx, |ui| {
                ui.set_max_width(450.0); ui.label(error); ui.add_space(12.0); dismiss = ui.button("Back to editing").clicked();
            });
...
impl eframe::App for Studio {
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        self.inspector(ctx); self.canvas(ctx);
...
fn slider(ui: &mut egui::Ui, label: &str, value: &mut f32, range: std::ops::RangeInclusive<f32>, suffix: &str) {
    ui.label(RichText::new(label).size(12.0));
    ui.add(egui::Slider::new(value, range).suffix(suffix).fixed_decimals(2)); ui.add_space(10.0);
}
fn theme(ctx: &egui::Context) {
    ...
    visuals.selection.bg_fill = Color32::from_rgb(74, 90, 50); visuals.selection.stroke = Stroke::new(1.0, ACCENT);
    ...
    ctx.style_mut(|style| { style.spacing.item_spacing = egui::vec2(8.0, 6.0); style.spacing.button_padding = egui::vec2(12.0, 7.0); style.spacing.slider_width = 170.0; style.spacing.interact_size.y = 28.0; });
}
#[cfg(test)] mod tests {
    use super::*;
    #[test] fn cursor_anchored_zoom_preserves_image_coordinate() {
        let pan = egui::vec2(21.0, -17.0); let anchor = egui::vec2(100.0, 82.0);
