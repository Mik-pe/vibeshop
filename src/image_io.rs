...
pub fn save_png(path: &Path, width: u32, height: u32, rgba: &[u8]) -> Result<()> {
    ...
    let mut temporary = tempfile::NamedTempFile::new_in(parent).context("Could not create export in destination directory")?;
    image::codecs::png::PngEncoder::new(temporary.as_file_mut()).write_image(rgba, width, height, image::ExtendedColorType::Rgba8)?;
    temporary.flush()?; temporary.as_file().sync_all()?;
    temporary.persist(path).context("Could not replace export destination; existing file was not truncated")?;
    Ok(())
}
