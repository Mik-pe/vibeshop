...
Source pixels are uploaded once per active source identity. Interactive color adjustment and compositing happens in WGSL. Two reusable 512×512 RGBA16F scratch textures ping-pong through layers intersecting a dirty tile, then a final pass updates that region of the display and export textures. Ordered per-tile signatures track source identity and pixel-affecting layer settings, including old/new bounds after movement. Unchanged tiles are reused. Sources, output textures and the frozen export snapshot are still full-image allocations; bounded residency remains scaling work.

## Color and alpha contract

Inputs are currently 8-bit, untagged sRGB PNG/JPEG. Embedded ICC profiles are rejected rather than silently misinterpreted. EXIF orientation is applied during decode. Higher-bit-depth input is reduced to 8-bit; this is not a professional color-managed or HDR workflow. Version 2 projects preserve these decoded RGBA8 assets, not the original encoded file or discarded metadata.

WGSL decodes sRGB to linear light, applies tone adjustments, and composites premultiplied RGB/alpha into RGBA16F. Source-over normal, multiply and screen have pixel tests. Levels follow exposure/contrast/saturation and map input black/white to 0/1 with reciprocal gamma (>1 lightens); active levels/curves clamp their domain to 0..=1. Neutral settings retain the existing unclipped linear pipeline. Sparse curves interpolate piecewise-linearly through monotone defined points, anchored implicitly at (0,0)/(1,1). The RGB curve runs before the separate R/G/B curves. GPU LUT sampling interpolates between 256 samples; this approximates the piecewise reference near control knots. Levels and curves do not modify alpha.

Final output converts straight linear RGB to sRGB. egui receives gamma-encoded premultiplied RGBA8; PNG receives straight-alpha RGBA8. Fully transparent exported RGB is normalized to zero. Do not apply sRGB transfer functions twice or blend source layers in gamma space.

The output-encode pass also replaces each changed tile's GPU histogram bins, using the same linear-pixel load. A reduction sums cached tile bins, including unchanged tiles, into a 4 KiB total. Only that total is read back; one worker can be in flight, tagged with its render generation. Histograms count each nontransparent pixel once, after unpremultiplication in the final linear composition and before output encoding. Luminance is 0.2126R+0.7152G+0.0722B. Values clamp to 0..=1 and map to floor(value*255); endpoint bins are counts, not evidence of pre-clamp clipping. At most 256 tiles retain 1 MiB of histogram bins at the existing size limits.

Display and PNG are produced from the same composition. Before comparison is a preview mode that bypasses every tonal adjustment; export first restores edited mode and renders it, without altering document state/history. Export freezes that revision in an immutable RGBA8 GPU texture before the file dialog. An IO worker copies horizontal bands through one reusable staging buffer capped at 1 MiB, strips row padding and feeds rows directly to the streaming PNG encoder. It finishes both the compressed stream and PNG container before atomically replacing the destination; transfer or encoding failure removes the temporary file and preserves the previous destination. Later edits, resize, or dialog cancellation cannot mix revisions into the export. Readback and image codecs run off the UI thread. No full-image readback happens on ordinary rendering; the live histogram reads only its 4 KiB counters. Failed rendering invalidates export rather than returning pixels from an older revision. Project saving does not need GPU readback.
...
At the maximum 16,777,216 pixels, two RGBA8 outputs plus the reusable 16F scratch tiles have a 132 MiB texture payload, excluding sources, driver overhead and readback. The frozen export snapshot adds up to 64 MiB of GPU texture payload under current limits. Export transfer storage is at most 1 MiB, plus PNG row/filter/compression storage and driver overhead; the application does not collect a full-image CPU pixel vector for PNG export. Full-image sources, output targets and snapshots still prevent a large-document guarantee. Do not raise limits without bounding those allocations and measuring memory and interaction latency. See [the compositor workload](RENDERING_PERFORMANCE.md) for reproducible measurements and their limits.
