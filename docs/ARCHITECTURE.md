...
Each changed tile also replaces its GPU histogram bins. A reduction sums cached tile bins, including unchanged tiles, into a 4 KiB total. Only that total is read back; one worker can be in flight, tagged with its render generation. Histograms count each nontransparent pixel once, after unpremultiplication in the final linear composition and before output encoding. Luminance is 0.2126R+0.7152G+0.0722B. Values clamp to 0..=1 and map to floor(value*255); endpoint bins are counts, not evidence of pre-clamp clipping. At most 256 tiles retain 1 MiB of histogram bins at the existing size limits.
